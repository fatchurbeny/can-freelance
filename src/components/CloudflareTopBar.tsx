...
export default function CloudflareTopBar({
  badgeLabel = 'DASHBOARD',
  periods,
  currentPeriod,
  actionButton,
  rightAction,
}: CloudflareTopBarProps) {
  ...
  // Match icon with exact sidebar menu route icon
  const getPageIcon = () => {
    switch (pathname) {
      case '/':
      ...
      <div className="w-full flex h-14 items-center">
        ...
        <div className="w-56 h-full shrink-0 flex items-center px-5 border-r border-[#f0f0f0] dark:border-[#272a34]">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-display text-sm font-bold tracking-tight text-[#262626] dark:text-white">
              impro.std
            </span>
          </Link>
        </div>
        ...
        <div className="flex-1 h-full flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-2">
            <PageIcon className="w-4 h-4 text-[#ff5e1f]" />
            <span className="font-display text-sm font-semibold tracking-wide text-[#262626] dark:text-white">
              {badgeLabel}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="hidden sm:flex items-center gap-3 rounded-lg border border-[#f0f0f0] dark:border-[#272a34] bg-gray-50 dark:bg-[#16181d] px-3 py-1.5 text-xs text-gray-400 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-700 transition-colors cursor-pointer">
              ...
              <span>Search</span>
              <kbd className="rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#0d0e12] px-1.5 py-0.5 font-mono text-[10px] text-gray-500 dark:text-gray-400">
                ⌘ K
              </kbd>
            </button>
