...
export const INDONESIAN_BANK_OPTIONS = [
  'BCA',
  'Mandiri',
  'BRI',
  'BNI',
...
interface Props {
  value: string;
  onChange: (newBank: string) => void;
  placeholder?: string;
  disabled?: boolean;
}

export default function BankSelectCell({
  value,
  onChange,
  ...
  const isPreset = INDONESIAN_BANK_OPTIONS.includes(value);
      ...
      {isCustomMode ? (
        <div className="w-full h-full min-h-[44px] flex items-center bg-gray-50/50 dark:bg-[#16181d]/50">
          <input
            ...
            className="w-full h-full min-h-[44px] rounded-none border-0 bg-transparent px-3 font-sans text-xs font-bold text-gray-900 dark:text-white outline-none focus:bg-white dark:focus:bg-[#16181d] focus:ring-1 focus:ring-[#ff5e1f] transition-colors uppercase"
          />
          <button
            type="button"
            onClick={() => setIsCustomMode(false)}
            title="Kembali ke Pilihan Dropdown"
            className="px-2.5 h-full flex items-center justify-center text-gray-400 hover:text-[#ff5e1f] transition-colors cursor-pointer shrink-0"
          >
        ...
        <button
          ...
          className="w-full h-full min-h-[44px] flex items-center justify-between px-3 bg-gray-50/50 dark:bg-[#16181d]/50 hover:bg-gray-100 dark:hover:bg-[#16181d] text-xs font-sans font-bold text-gray-900 dark:text-white transition-colors cursor-pointer select-none disabled:opacity-50"
        >
        ...
        <div className="absolute left-0 right-0 top-full z-50 mt-0 rounded-none border border-[#f0f0f0] dark:border-[#272a34] bg-white dark:bg-[#16181d] shadow-xl font-sans text-xs flex flex-col p-1.5 space-y-0.5 max-h-60 overflow-y-auto">
          {INDONESIAN_BANK_OPTIONS.map((bank) => {
            ...
            return (
              ...
              >
                <span className="truncate uppercase font-bold">{bank}</span>
                <div
